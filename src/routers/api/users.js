...
const { findUserByIdAndAttrs, findUserByIdAndIncludes, deleteAuthToken } = require("../../controllers/user");
...
router.get("/me",
    ...
    async function (req, res) {
        ...
        if (req.user && !req.authInfo.clientOnly && req.user.id) {
            ...
            try {
                const user = await findUserByIdAndIncludes(req.user.id, includes);
                if (!user) {
...
router.get("/:id",
    ...
    async function (req, res) {
        ...
        try {
            ...
            const user = await findUserByIdAndAttrs(req.params.id, attributes);
            if (!user) {
                throw new Error("User not found");
            }
            res.send(user);
